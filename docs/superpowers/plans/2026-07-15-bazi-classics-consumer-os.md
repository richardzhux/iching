...
- [x] Add an ignored classics cache and a deterministic repository-size guard with 1.85 GB soft and 2 GB hard thresholds.
- [x] Remove `_theme_score`, `score_bazi_consumer_themes`, `_PATTERN_THEME_BONUS`, and the 43-point/28–98 scoring path from live profile generation.
- [x] Replace score-bearing subject payloads with descriptive path fields and optional directional/incidence metadata.
- [x] Preserve legacy optional TypeScript fields only for rendering old snapshots; new builders must not populate them.
- [x] Add focused assertions that no new chart returns naked life-quality scores and that hidden wealth is described as an expression path, not poverty.
- [x] Run the focused Python contract tests and commit `refactor: remove life quality scores and guard workspace budget`.
...
- [x] Download the public-domain *Gengcun Ji*, the 1926 Qin Shen'an volume-two operational witness, and the independent World Library witness into ignored `references/classics/raw/` and record SHA-256 values.
- [x] Treat 8bei8, Dongli, CText, Wikisource, and unlicensed GitHub mirrors as search aids only; no production rule may depend solely on them.
- [x] Preserve diplomatic traditional text separately from normalized search text; OpenCC output never overwrites the witness text.
- [x] Complete two end-to-end chapters: `论用神成败救应` and `论正官`, including original text, commentary boundary, atomic propositions, examples, and non-rule classification.
- [x] Visually verify every quote used by a production proposition against a scan.
- [x] Run source-manifest validation and commit `data: establish ziping vertical source corpus`.
...
- [x] Centralize stems, branches, hidden stems, elements, ten gods, and relation primitives without changing existing public imports.
- [x] Record explicit pillar positions, month qi levels, exposed/hidden occurrences, roots, relations, complete combinations, and input completeness without pattern judgment.
- [x] Implement three-valued predicates: `all`, `any`, `not`, `fact_equals`, `fact_in`, `exists_occurrence`, `count_compare`, `relation_exists`, `root_exists`, `month_command_equals`, `god_exposed`, `combination_complete`.
- [x] Reject source-less production rules, invalid locators/hashes, duplicate IDs, unknown operators, cycles, and unresolved precedence.
- [x] Prove two compilations produce the same digest and commit `feat: add source-backed bazi rule compiler`.
...
- [x] Bind each damage to a specific formation path and accept rescue only when it resolves that actual damage.
- [x] Encode direct-officer candidate, wealth/resource formation paths, hurting-officer damage, resource rescue, officer/killing mixing, and explicit conflicts from the reviewed propositions.
- [x] Remove list-order fallbacks and artificial effectiveness/share thresholds from the new path.
- [x] Build the fact graph once per chart and share it with structure and pattern evaluation.
- [x] Shadow the new result beside the legacy result, classify every focused difference, and commit `feat: compile direct-officer lifecycle from ziping sources`.
...
- **Reviewed special-pattern scope (2026-07-16):** this milestone delivers the ordinary-pattern registry plus a strict ordinary-before-special review gate. The verified appendix examples and rejected candidate rules are retained, but 从财、从杀、从儿、从旺、从强 and other special patterns cannot become active until their complete candidate, formation, rejection, and precedence predicates are source-bound. The engine must return no special verdict rather than infer one from an example label.

- [x] Expand the Shen bundle through seven killings, wealth, resource, output, month prosperity/robbery, and yang blade; add the strict fail-closed special-pattern review gate without claiming unsupported special formations.
- [x] Give every rule a positive case, missing-condition case, damage case, rescue case where applicable, precedence case, and traceable proposition.
- [x] Compile Xu commentary only where a dated witness can be verified; exclude later annotations and unresolved attribution.
...
- [x] Preserve author claims separately from current engine expectations in classical examples.
- [x] Commit `feat: complete independent ziping classical rule layers`.
...
- [x] Generate exactly one pattern identity, four expression paths, three-to-five non-duplicate signature structures, zero-to-four meaningful combinations, and three future windows for a complete chart.
- [x] Order by classical importance first and statistical distinctiveness second; a rare single ShenSha cannot outrank the month-command pattern.
- [x] Replace headline keyword guessing with stable backend `path_id` and claim IDs.
- [x] Rename achievements to rare structure combinations; single ordinary ShenSha stays in the full chart.
- [x] Return concise consumer copy first and source/evidence IDs for progressive disclosure.
- [x] Commit `feat: compile chart facts into consumer claims`.
...
- **Reviewed scope correction (2026-07-16):** the production period engine computes deterministic Ten-God, ShenSha, and stem/branch-relation activations for Da Yun, year, and month. It does not label transit formations, damage, or rescue until a separately reviewed period-doctrine bundle and typed temporal fact overlay exist. The consumer matcher rejects fabricated or cross-path lifecycle provenance, and the K-line remains fail-closed when no canonical transition is produced.

- [x] Stop displaying exact-tie percentage as the default comparison; use tail probability for ordered metrics.
- [x] Suppress precise rankings when same-mass exceeds 25%; use broad directional copy for 10–25% and exact `前约 X%` only below 10%.
- [x] Remove default overall K-line and retain career, wealth, relationship, and rhythm series.
- [x] Compute monthly activation from the natal chart + Da Yun + year + month with deterministic OHLC and named Ten-God, ShenSha, and relation drivers; keep formation/damage/rescue absent rather than infer unsupported transit doctrine.
- [x] Put three key windows before the chart; default to a simple trend, with professional candlesticks one action away.
- [x] Add the pattern bundle digest to the statistical registry and generate g4 baselines only after the bundle freezes.
- [x] Commit `feat: align statistics and kline with consumer semantics`.
...
- [x] Render pattern title, formation path, status, one memorable sentence, and three compact tags in the hero.
- [x] Add one relative-position or incidence highlight and next activation to each theme path.
- [x] Show the pattern decision tree with fact -> rule -> proposition -> scan progression.
- [x] Merge exports under one share menu for identity, rare combination, future window, and full report.
- [x] Keep tables internally scrollable and eliminate page-level overflow at 390 px.
- [x] Commit `feat: deliver the bazi consumer operating system`.
...
- [x] Include rule bundle digest in live charts, saved snapshots, exports, and statistical compatibility checks.
- [x] Return source-backed rule summaries on demand without embedding the corpus or full trace in each snapshot.
...
parseable_source_examples_without_structured_record == 0
naked_life_scores == 0
