...
def test_source_witnesses_freeze_identity_rights_and_raw_provenance() -> None:
    ...
    assert all(aid["production_use_allowed"] is False for aid in manifest["search_aids"])
...
def test_vertical_slice_is_complete_deterministic_and_scan_traceable() -> None:
    ...
    assert {chapter["id"]: chapter["title_diplomatic"] for chapter in manifest["chapters"]} == EXPECTED_CHAPTERS
    ...
    assert all(set(item["base_locator_ids"] + item["witness_locator_ids"]) <= set(locators) for item in variants)
    assert all(item["witness_id"] in witnesses for item in variants)
    material_pairs = {(item["base_reading"], item["witness_reading"]) for item in variants}
    ...
    for chapter in manifest["chapters"]:
        ...
        assert {kind: len(records) for kind, records in {
            "segments": segments,
            "propositions": propositions,
            "rule_candidates": rules,
            "examples": examples,
            "non_rules": non_rules,
        }.items()} == chapter["record_counts"]
        ...
        assert all(item["diplomatic_text"] and item["normalized_search_text"] for item in segments)
        assert any(item["diplomatic_text"] != item["normalized_search_text"] for item in segments)
        assert all(item["layer"] in {"shen_core", "editorial", "commentary", "example"} for item in segments)
        ...
        if chapter["id"] == "zzq.useful-god.success-failure-rescue":
            formation = next(item for item in segments if item["id"] == "zzq.seg.useful.formation")
        ...
        for proposition in propositions:
            ...
            assert proposition["explicit_conditions"] or proposition["inferred_conditions"]
            if proposition["production_eligible"]:
                ...
                for locator_id in proposition["locator_ids"]:
                    ...
                    assert witnesses[locator["witness_id"]]["rights"]["production_use_allowed"] is True
        ...
        assert all(propositions_by_id[item["proposition_id"]]["production_eligible"] is True for item in rules)
        unresolved = [item for item in rules if item["semantic_status"].startswith("requires_")]
        assert unresolved
        assert all(item["execution_ready"] is False for item in unresolved)
        assert all(item["candidate_status"] == "source_verified_not_compiled" for item in unresolved)
    ...
    assert len(all_segment_ids) == sum(chapter["record_counts"]["segments"] for chapter in manifest["chapters"])
    assert len(all_proposition_ids) == sum(chapter["record_counts"]["propositions"] for chapter in manifest["chapters"])
